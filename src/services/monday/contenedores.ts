...
export async function opcionesDeContenedor(): Promise<OpcionContenedor[]> {
  const datos = await mondayApi<{ boards: { items_page: { items: ItemCrudo[] } }[] }>('contenedores', {
    columnas: COLUMNAS,
    limite: 200,
  })

  return (datos.boards?.[0]?.items_page.items ?? [])
    .map((item): OpcionContenedor => {
      ...
      return {
        ...
        capacidad: Math.max(0, Math.trunc(aNumeroEspejo(texto(c[COL_CONTENEDOR.capacidad])) ?? 0)),
